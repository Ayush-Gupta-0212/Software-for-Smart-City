# smart-city — Terraform Infrastructure

Production-grade IaC for smart-city on AWS. Provisions a free-tier-friendly stack:
...
```bash
aws s3api create-bucket --bucket smart-city-tf-state --region us-east-1
aws s3api put-bucket-versioning --bucket smart-city-tf-state \
  --versioning-configuration Status=Enabled
aws s3api put-bucket-encryption --bucket smart-city-tf-state \
  --server-side-encryption-configuration '{"Rules":[{"ApplyServerSideEncryptionByDefault":{"SSEAlgorithm":"AES256"}}]}'
aws dynamodb create-table --table-name smart-city-tf-lock \
...
# Deploy smart-city via Helm (from local laptop):
KUBECONFIG=./kubeconfig helm upgrade --install smart-city ../helm/smart-city \
  -f ../helm/smart-city/values-prod.yaml \
  --namespace smart-city --create-namespace --wait
```
